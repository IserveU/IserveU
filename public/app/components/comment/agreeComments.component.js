(function() {
  ...
  angular
    .module('app.comment')
    .directive('agreeCommentsComponent', {
      require: { parent: '^^commentListComponent' },
      template: `
      <md-list ng-show="$ctrl.parent.motion.motionComments.agreeComments.length != 0">
        <md-list-item class="md-3-line md-long-text motion_commentlist__item" ng-repeat="comment in $ctrl.parent.motion.motionComments.agreeComments | orderBy:'-commentRank'">
      ...
      <section ng-show="$ctrl.parent.motion.motionComments.agreeComments.length == 0">
        <md-icon class="mdi mdi-microphone-off title-icon"></md-icon>
        <h1 class="md-subhead title-header" translate="COMMENTLIST_NOAGREE"></h1>
      </section>
    `
    });

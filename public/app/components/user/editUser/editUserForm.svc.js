(function() {
	...
	function editUserFormServiceFactory($http, $filter, communityResource, utils) {
			...
			personalInformation.forEach(function(el, index) {
				switch(el.label) {
					case 'Name':
						...
						break;
					case 'Community':
						communityResource.retrieveNameById(user.community_id).then(function(name){
							el.data = name;
						});
						break;
					case 'Birthday':
					...
					case 'Address':
						if( !user.street_name )
							return;
	      					...
	      					el.data = user.street_number + ' ' + utils.toTitleCase(user.street_name);
	      				else if ( !user.street_number )
	      					el.data = "Unit #" + user.unit_number + ' ' + utils.toTitleCase(user.street_name);
	      				else
							el.data = user.unit_number + '-' + user.street_number + ' ' + utils.toTitleCase(user.street_name) + ( user.postal_code ? ', ' + user.postal_code : '');
						break;
				}
			});

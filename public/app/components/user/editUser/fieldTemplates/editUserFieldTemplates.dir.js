	...
	function editBirthdayField() {
		return {
			template: ['<form layout="row"  ng-controller="birthdayController as birthday" flex>',
			           '<md-input-container>',
          			   '<label>Month</label>',
					   '<md-select ng-model="profile.date_of_birth.month" ng-change="selectMonth(profile.date_of_birth.month,profile.date_of_birth.year)"><md-option ng-repeat="month in months " placeholder="Month" ng-value="month">{{month.name}}</md-option></md-select>',
			           '</md-input-container>',
          			   '<md-input-container>',
          			   '<label>Day</label>', 

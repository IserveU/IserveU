...
(function(window, angular, undefined) {
  ...
  angular
    ...
    .config(['$mdThemingProvider', 'SETTINGS_INIT',
      function($mdThemingProvider, SETTINGS_INIT) {
        var name = SETTINGS_INIT.theme.name || 'default';

        var theme = chooseTheme();

        function chooseTheme() {
          //customTheme mode on
          if (SETTINGS_INIT.theme.customTheme === 1) {
            return SETTINGS_INIT.theme.colors;
          } else { 
            return SETTINGS_INIT.theme.predefined;
          }
        }

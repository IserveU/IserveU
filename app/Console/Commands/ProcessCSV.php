...
class ProcessCSV extends Command
{
    ...
    public static function filterUnusableRecords(RowCollection $collection)
    {
        return $collection->filter(function ($filter) {
            if (empty($filter->email) && empty($filter->email1) && empty($filter->email2)) {
                return false;
            }
    ...
    public static function processUser(CellCollection $result)
    {
        ...
        if ($user) {
            ...
            if (!$user->middle_name && $result->middle_name) {
            ...
            if (!$user->street_name && $address) {
                $user->street_name = $address;
            }
            if (!$user->community_id && ($result->primary_city == 'Yellowknife')) {
                $user->community_id = 1;
            }
